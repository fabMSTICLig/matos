...
<template>
  <div v-if="firstCheck">
    <Header />
    <router-view :key="$route.name" class="p-4 corps" />
    <Footer />
  </div>
  <modal
    id="modal-cookie"
  ...
  >
    <p>
      ...
      utilisés pour gerer la session de l'utilisateur. Il n'y a pas de cookie de
      tierces parties ou d'utilisation à des fins statistiques.
    </p>
  ...
  <svg style="display: none" version="2.0">
    <defs>
      ...
      <symbol id="eye" viewBox="0 0 20 20">
        ...
        ></path>
        <circle fill="currentColor" cx="9.98" cy="9.446" r="1.629"></circle>
      </symbol>
    </defs>
  </svg>
</template>
...
const { firstCheck, isAuthenticated } = storeToRefs(store);

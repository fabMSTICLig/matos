...
<div>
  ...
  <div class="row">
    <div class="col-xl-3">
      ...
      <form class="form">
        <div class="form-group"><label>Chercher</label><input type="search" class="form-control" v-model="search_input" /></div>
    ...
    <div class="col">
      <div class="card">
        ...
        <div class="form-group"><label>Ordre : </label>
          <select class="form-control" v-model="sort_input">
            <option v-for="item in sort_choices"  :value="item.value" :key="item.value">{{item.label}}</option>
          </select>
        </div>
        </form>
          ...
          <ul class="list-group list-group-flush">
            <li
              ...
              <div class="d-flex w-100 justify-content-between">
                <h4>{{ item.name }}</h4><strong>{{ getEntityName(item.entity) }}</strong></div>
            ...
            <button class="btn btn-primary wt-1" type="button" @click="addMaterial(item)" :class="{disabled:pending_loan.entity && pending_loan.entity != item.entity}" title="Les matériel d'un prêt doivent tous appartenir à la même entité">Ajouter</button>
            </li>
          </ul>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      current_page: 1,
      entities_filter : [],
      sort_choices:{'name':{'value':1, 'label':"Nom"},'entity':{'value':2, 'label':"Entité"}}
    };
  },
  computed: {
    ...mapGetters({entities : "entities/list", 
    ...
    objects_list() {
      if(this.type_input == 2)
        ...
        return this.specificmaterials
      else{
        if(this.genericmaterials)
            return this.genericmaterials.concat(this.specificmaterials).sort((a, b) => {
    ...
    objects_filtered() {
        ...
        return filtered.sort((a,b)=>{
            ...
            if(this.sort_input == this.sort_choices.entity.value)
                return this.entityById(a.entity).name.localeCompare(this.entityById(b.entity).name)
        })
    },
    objects_paginated() {

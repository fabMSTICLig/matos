      ...
      if (typeof this.search_fields === 'function') {
        ...
        return this.objects_list.filter(item => {
          return this.search_fields.some((field) => {
            return item[field].toLowerCase().indexOf(this.search_input.toLowerCase()) > -1
          });
        })
      }
    },
    objects_paginated() {
      return this.objects_filtered.slice((this.current_page - 1) * process.env.VUE_APP_MAXLIST, (this.current_page) * process.env.VUE_APP_MAXLIST);
    },
    pages_count() {
      return Math.ceil(this.objects_filtered.length / process.env.VUE_APP_MAXLIST);
    }
  },
  methods: {
    onPageChange(page) {
  ...
  methods: {
    ...
    initObject(route) {
      if (route.params.id == "new") {
        ...
        this.$store.dispatch(this.ressource + '/fetchSingle', route.params.id).then((data) => {
    ...
    update() {
      this.$store.dispatch(this.ressource + '/update', {
        id: this.object.id,
        data: this.object
      }).then((data) => {
        this.object = Object.assign({}, data);
    ...
    create() {
      this.$store.dispatch(this.ressource + '/create', this.object).then((data) => {
    ...
    destroy() {
      this.$store.dispatch(this.ressource + '/destroy', this.object.id).then(() => {
        this.$router.push({
          name: this.ressource
        })
      });

<template>
    ...
    <div class="card m-2">
        ...
        <div v-if="show" class="card-body">
            ...
            <div class="mb-3">
                <select id="yearInput" v-model="yearInput" class="form-inline-control" @change="setDateWithYear()">
                    <option value="" selected> Choix par Année </option>
                    <option v-for="year in 3" :value="(dDate.getFullYear()+1-year).toString()"> {{ dDate.getFullYear()+1-year }} </option>
                </select>
            </div>
        </div>
    </div>

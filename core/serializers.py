...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        # remove if not using ldap info update : 'email', 'first_name',
        # 'last_name'
        read_only_fields = ('username', 'email', 'first_name', 'last_name', 'is_pro')
...
class LoanSerializer(serializers.ModelSerializer):
    ...
    """
    generic_materials = LoanGenericItemSerializer(
        source="loangenericitem_set", many=True)
    ...
    class Meta:
        ...
        extra_kwargs = {
            'status': {
                'error_messages': {
                    'invalid_choice': 'Veuillez séléctionner un status pour le prêt'}}}
    ...
    def update(self, instance, validated_data):
        ...
        for item in loangen:
            loan_item, created = LoanGenericItem.objects.get_or_create(
                material=item['material'], loan=instance)
    ...
    def validate(self, data):
        ...
        if ('parent' in data and data['parent'] is not None):
            ...
            if (data['checkout_date'] < loan_parent.checkout_date):
                raise serializers.ValidationError(
                    "La date de sortie doit être postérieure à celle du prêt parent " + str(
                        loan_parent.checkout_date))
        if (data['checkout_date'] > data['due_date']):
            raise serializers.ValidationError(
                "La date de rendu doit être après la date de sortie")
        # Si la date de retour est défini
        if (data['return_date'] and data['checkout_date']
                > data['return_date']):
            raise serializers.ValidationError(
                "La date de retour doit être après la date de sortie")
        if (len(data['specific_materials']) == 0 and len(
                data['loangenericitem_set']) == 0):
            raise serializers.ValidationError(
        ...
        for mat in data['specific_materials']:
            if mat.model.entity != entity:
                raise serializers.ValidationError(
                    "Tout les matériels doivent apartenir à l'entité preteuse.")
        for item in data['loangenericitem_set']:
            if item['material'].entity != entity:
                raise serializers.ValidationError(
                    "Tout les matériels doivent apartenir à l'entité prêteuse.")
        # SPEC MAT
        # conflit prêts en cours
        loans = Loan.objects.filter(
            specific_materials__in=data['specific_materials'],
            status=Loan.Status.ACCEPTED,
            checkout_date__lte=data['checkout_date'],
            return_date=None,
            due_date__gt=data['checkout_date']).distinct()
        if ('id' in self.initial_data):
            loans = loans.exclude(id=self.initial_data['id'])
        for loan in loans:
            materialintersec = [
                x.name for x in loan.specific_materials.all() if x in data['specific_materials']]
            raise serializers.ValidationError("Prêt en cours pour le matériel suivant : " + str(
                materialintersec[0]) + " - jusqu'au " + str(loan.due_date))
        # conflit prêts en fini
        loans = Loan.objects.filter(
            specific_materials__in=data['specific_materials'],
            status=Loan.Status.ACCEPTED,
            checkout_date__lte=data['checkout_date'],
            return_date__gt=data['checkout_date'])
        if ('id' in self.initial_data):
            loans = loans.exclude(id=self.initial_data['id'])
        for loan in loans:
            materialintersec = [
                x.name for x in loan.specific_materials.all() if x in data['specific_materials']]
            raise serializers.ValidationError("Prêt en cours pour le matériel suivant : " + str(
                materialintersec[0]) + " - jusqu'au " + str(loan.return_date))
        # conflit prêts dans le future
        loans = Loan.objects.filter(
            specific_materials__in=data['specific_materials'],
            status=Loan.Status.ACCEPTED,
            checkout_date__gte=data['checkout_date'])
        if ('id' in self.initial_data):
            loans = loans.exclude(id=self.initial_data['id'])
        for loan in loans:
            if data['return_date']:
                if data['return_date'] > loan.checkout_date:
                    materialintersec = [
                        x.name for x in loan.specific_materials.all() if x in data['specific_materials']]
                    raise serializers.ValidationError("Le matériel " +
                                                      str(materialintersec[0]) +
                                                      " doit être retourné avant le " +
        ...
        for item in data['loangenericitem_set']:
            ...
            if (item['material'].quantity < item['quantity']):
                raise serializers.ValidationError(
                    "Pas assez de " + item['material'].name + " en stock " + str(
                        item['material'].quantity) + " max")
            loanslist = []
            structlist = []
            # Creation d'une structure contenant l'ensemble des prêts checvauchant le prêt courant
            # conflit prêt en cours
            loans = Loan.objects.filter(
                generic_materials__id=item['material'].id,
                status=Loan.Status.ACCEPTED,
                checkout_date__lte=data['checkout_date'],
                return_date=None,
                due_date__gt=data['checkout_date']).distinct()
            ...
            if data['return_date']:
                loans = Loan.objects.filter(
                    generic_materials__id=item['material'].id,
                    status=Loan.Status.ACCEPTED,
                    checkout_date__range=(
                        data['checkout_date'],
                        data['return_date'])).distinct()
            else:
                loans = Loan.objects.filter(
                    generic_materials__id=item['material'].id,
                    status=Loan.Status.ACCEPTED,
                    checkout_date__range=(
                        data['checkout_date'],
                        data['due_date'])).distinct()
            if ('id' in self.initial_data):
                loans = loans.exclude(id=self.initial_data['id'])
            loanslist.extend(loans)
            for loan in loanslist:
                quantity = loan.loangenericitem_set.get(
                    material=item['material']).quantity
                struct = {
                    'id': loan.id,
                    'quantity': quantity,
                    'checkout_date': loan.checkout_date,
                    'end_date': loan.return_date if loan.return_date else loan.due_date}
            ...
            structlist.append(
                {
                    ...
                    'end_date': data['return_date'] if data['return_date'] else data['due_date']})
            ...
            while i < lenlist:
                ...
                if index != -1:
                    ...
                    while y < lenlist:
                        if i != y and structlist[i]['checkout_date'] >= structlist[y][
                                'checkout_date'] and structlist[i]['checkout_date'] < structlist[y]['end_date']:
                            total = total + structlist[y]['quantity']
                        y = y + 1
                    if (total > item['material'].quantity):
                        maxtot = maxtot if total - \
                            item['material'].quantity <= maxtot else total - item['material'].quantity
                i = i + 1
            if (maxtot > 0):
                raise serializers.ValidationError(
                    "Pas assez de quantité en stock pour " +
                    item['material'].name +
                    " ( " +
                    str(maxtot) +
                    " manquant(s) ) pour les dates selectionnées")
...
class LoanNestedSerializer(serializers.ModelSerializer):
    ...
    generic_materials = LoanGenericItemStringSerializer(
        source="loangenericitem_set", many=True)
